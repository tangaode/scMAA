"""Main orchestration for scRNA + spatial analysis."""
...
class ScRSTAgent:
    """Research-oriented agent for integrated scRNA + spatial analysis."""
    ...
    def __init__(
        ...
        *,
        ...
        self.spatial_h5ad_path = str(Path(spatial_h5ad_path).resolve())
        ...
        self.prompt_dir = Path(prompt_dir) if prompt_dir else self.project_root / "scrst_agent" / "prompts"
        ...
        self.spatial_summary = self._summarize_spatial_data(self.spatial_h5ad_path)
        self.joint_summary = self._summarize_joint_data(self.rna_h5ad_path, self.spatial_h5ad_path)
        self.dataset_validation = DatasetValidator().inspect_inputs(self.rna_h5ad_path, self.spatial_h5ad_path)
        ...
        self.hypothesis_generator = HypothesisGenerator(
            ...
            tcr_summary=self.spatial_summary,
        ...
        self.executor = LegacyNotebookExecutor(
            ...
            spatial_h5ad_path=self.spatial_h5ad_path,
    ...
    def _build_publication_figure(self) -> FigureResult:
        ...
        result = build_publication_figure(
            ...
            mapping_method=self.spatial_mapping_method,
        )
        ...
        if self.dataset_validation.warnings:
            ledger.open_questions.extend(
                [
                    ...
                    "What is the strongest spatial analysis that can survive the listed guardrails?",
                ]
            )
        return ledger
    ...
    def _summarize_spatial_data(self, spatial_h5ad_path: str) -> str:
        import anndata as ad

        adata = ad.read_h5ad(spatial_h5ad_path, backed="r")
        try:
            obs_columns = [str(col) for col in adata.obs.columns]
            metadata_lines: list[str] = []
            lower_to_original = {col.lower(): col for col in obs_columns}
            for hint in SPATIAL_METADATA_HINTS:
                column = lower_to_original.get(hint)
                if not column:
                    continue
                metadata_lines.append(
            ...
            lines = [
                f"Spatial file: {spatial_h5ad_path}",
                f"Spatial matrix shape: {adata.n_obs} spots x {adata.n_vars} genes",
                f"obs columns ({len(obs_columns)}): {', '.join(obs_columns[:30]) or 'none'}",
                f"obsm keys: {', '.join(list(adata.obsm.keys())[:20]) or 'none'}",
                f"layers: {', '.join(list(adata.layers.keys())[:20]) or 'none'}",
                f"uns keys: {', '.join(list(adata.uns.keys())[:20]) or 'none'}",
                f"spatial coordinates present: {'spatial' in adata.obsm}",
                f"raw present: {adata.raw is not None}",
            ]
            if metadata_lines:
                lines.append("Candidate spatial metadata columns:")
                lines.extend(metadata_lines)
            return "\n".join(lines)
        finally:
            if getattr(adata, "file", None) is not None:
    ...
    def _summarize_joint_data(self, rna_h5ad_path: str, spatial_h5ad_path: str) -> str:
        import anndata as ad

        rna = ad.read_h5ad(rna_h5ad_path, backed="r")
        spatial = ad.read_h5ad(spatial_h5ad_path, backed="r")
        try:
            shared_genes = sorted(set(map(str, rna.var_names)).intersection(map(str, spatial.var_names)))
            sample_lines: list[str] = []
            rna_sample_col = infer_sample_column(rna.obs.columns)
            spatial_sample_col = infer_sample_column(spatial.obs.columns)
            if rna_sample_col:
                sample_lines.append(
                    f"RNA sample column selected for stratified analyses: {rna_sample_col} ({rna.obs[rna_sample_col].nunique(dropna=True)} groups)."
                )
            if spatial_sample_col:
                sample_lines.append(
                    f"Spatial sample column selected for stratified analyses: {spatial_sample_col} ({spatial.obs[spatial_sample_col].nunique(dropna=True)} groups)."
                )
            ...
            if "tissue" in spatial.obs.columns:
                sample_lines.append(f"Spatial tissue groups: {spatial.obs['tissue'].nunique(dropna=True)}; top levels: {_top_counts(spatial.obs['tissue'])}")
            lines = [
                f"Shared genes between RNA and spatial objects: {len(shared_genes)}",
                f"RNA cells: {rna.n_obs}",
                f"Spatial spots: {spatial.n_obs}",
                f"Approximate shared-gene fraction relative to RNA genes: {(len(shared_genes) / max(rna.n_vars, 1)):.3f}",
                f"Approximate shared-gene fraction relative to spatial genes: {(len(shared_genes) / max(spatial.n_vars, 1)):.3f}",
            ]
            lines.extend(sample_lines)
            lines.append(
                "Notebook objects that will exist after setup: adata_rna, adata_spatial, shared_genes, "
                "and helper functions for marker transfer, spatial coordinates, tumor-like subsets, and domain DE."
            )
            return "\n".join(lines)
        finally:
    ...
    def _finalize_run_outputs(
        ...
        *,
        ...
        summary_lines = [
            ...
            f"Spatial input: {self.spatial_h5ad_path}",
            f"Research brief input: {self.research_brief_path}",
            f"Spatial mapping method: {self.spatial_mapping_method}",
